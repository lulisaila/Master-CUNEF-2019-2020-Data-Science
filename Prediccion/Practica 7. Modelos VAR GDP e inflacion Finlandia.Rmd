---
title: "Practica VAR GDP Inflation"
...
library("quantmod") #Package to download financials historical data # para bajarme los datos
library(forecast) # para hacer la prediccion
library("fGarch")
library(tidyverse)
library(skimr)
```

Gross Domestic Product by Expenditure in Constant Prices: Total Gross Domestic Product for Finland: https://fred.stlouisfed.org/series/NAEXKP01FIQ657S 

Consumer Price Index: All Items for Finland: https://fred.stlouisfed.org/series/FINCPIALLQINMEI

```{r}
series <- c('NAEXKP01FIQ657S', 'FINCPIALLQINMEI') # FRED codes for US GDP growth and CPI
...
```{r}
centre_colour = c("turquoise","purple") # Colours for time series/centre of fancharts
...
pivot_longer(Data, cols = -Date, names_to = "Variables", values_to = "Values") %>% 
  ...
  labs(title = "Finland GDP growth and CPI inflation", x = "", y = "",
...
ggplot(forecast_data) + 
  ...
  labs(title = "Forecasts of Finland GDP growth and CPI inflation", 
